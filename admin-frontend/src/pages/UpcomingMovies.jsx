...
const UpcomingMovies = () => {
    ...
    const handleDelete = async (id) => {
        if (!window.confirm('Are you sure you want to delete this upcoming movie? This action cannot be undone.')) return;
        try {
    ...
    return (
        <div className="space-y-8 fade-in h-full flex flex-col">
            <div className="flex justify-between items-end">
                <div>
                    <h1 className="text-3xl font-bold tracking-tight text-base-50 mb-2">Upcoming Movies</h1>
                    <p className="text-base-400">Manage movies marked as upcoming.</p>
                </div>
            ...
            <div className="box-panel flex-1 overflow-hidden flex flex-col">
                {isLoading ? (
                    ...
                    <div className="flex-1 flex flex-col items-center justify-center text-center p-12">
                        ...
                        <h3 className="text-lg font-medium text-base-100 mb-1">No upcoming movies found</h3>
                        <p className="text-base-400">Add movies and mark them as upcoming to see them here.</p>
                    </div>
                ) : (
                        ...
                        <table className="w-full text-left border-collapse">
                            <thead>
                                <tr className="border-b border-base-800/60 bg-base-900/30">
                                    ...
                                    <th className="py-4 px-6 text-xs font-semibold text-base-400 uppercase tracking-wider">Trailer</th>
                            ...
                            <tbody className="divide-y divide-base-800/60">
                                {movies.filter(m => m.title.toLowerCase().includes(searchTerm.toLowerCase())).map((movie, index) => (
                                    ...
                                    >
                                        ...
                                        <td className="py-4 px-6">
                                            {movie.trailerUrl ? (
                                                <a href={movie.trailerUrl} target="_blank" rel="noreferrer" className="text-primary-400 hover:text-primary-300 text-sm">
                                                    View Trailer
                                                </a>
                                            ) : (
                                                <span className="text-base-500 text-sm">No Trailer</span>
                                            )}
                                        </td>
