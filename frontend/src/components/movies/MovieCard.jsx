...
const MovieCard = ({ movie, index = 0 }) => {
  ...
  return (
    <>
      <Link to={`/movie/${movie._id}`} className="block w-full">
        <m.div
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
          transition={{ delay: index * 0.04, duration: 0.35 }}
          className="group relative w-full aspect-2/3 rounded-lg overflow-hidden bg-base-900 border border-base-800/60 hover:border-primary-500/50 transition-all duration-500 shadow-sm hover:shadow-lg hover:shadow-primary-500/5 cursor-pointer"
        >
          {/* Poster */}
          <img
